...
SOURCE_MODE = os.environ.get('CONC_SOURCE_MODE', 'blob').lower() 
...
output_timeseries_html = r"C:/Users/JR13/Downloads/counts_timeseries_ordered.html"
...
def iter_counts_csv_from_folder(folder: str):
    """Yield (name, bytes) for files in a local folder that end with 'counts.csv'."""
    p = Path(folder)
    if not p.exists():
...
def iter_counts_csv_from_blob():
    """Yield (name, bytes) from Azure blob container for blobs ending with 'counts.csv'."""
    from azure.storage.blob import BlobServiceClient  # import only when needed
    ...
    for blob in container_client.list_blobs():
        ...
        yield blob.name.split('/')[-1], content
...
if SOURCE_MODE == 'folder':
    iterator = iter_counts_csv_from_folder(LOCAL_COUNTS_DIR)
elif SOURCE_MODE == 'blob':
    iterator = iter_counts_csv_from_blob()
else:
...
def load_cytoclus_file(path):
    """Load a cytoclus .xlsx file and return a dataframe with 'fname' plus counts."""
...
count_cols_cytoclus = [c for c in cytoclusxlsx.columns if c.startswith("Counts_")]
...
# -----------------------------
# Per-class total differences & % diff (master vs cytoclus)
# -----------------------------
if len(shared) > 0 and not joined.empty:
    # Build per-side column names present in the joined table
    ...
    # Union of classes seen on either side (keeps you safe if columns differ)
    ...
    # Absolute difference (master - cytoclus)
    summary_df["diff"] = summary_df["total_master"] - summary_df["total_cytoclus"]

    # Percent difference relative to cytoclus (reference)
    # %diff = 100 * (master - cytoclus) / max(|cytoclus|, eps)
    eps = 1e-12
    denom = summary_df["total_cytoclus"].abs().clip(lower=eps)
    summary_df["pct_diff"] = 100.0 * summary_df["diff"] / denom

    # Add a +/- formatted string for quick pasting into docs
    summary_df["pct_diff_str"] = summary_df["pct_diff"].map(lambda x: f"{x:+.1f}%")

    # Optional: also include percentage relative to master (if you ever want both views)
    # denom_master = summary_df["total_master"].abs().clip(lower=eps)
    # summary_df["pct_diff_vs_master"] = 100.0 * summary_df["diff"] / denom_master

    # Nice ordering: biggest relative deviations first
    summary_df = summary_df.sort_values(by="pct_diff", key=lambda s: s.abs(), ascending=False)

    # Save to CSV next to your other outputs
    ...
    # Pretty console printout (rounded)
    ...
    with pd.option_context("display.max_rows", None, "display.float_format", "{:,.3f}".format):
        print("\n=== Per-class totals and differences (master vs cytoclus) ===")
