...
from flowcytometer_tool.tabs.continuous_sample_analyser.reporting_core import (
    apply_sampling_protocol_mutations,
    extract_prediction_calibration_packet_updates,
    write_report_packet_flat,
)
...
from flowcytometer_tool.tabs.continuous_sample_analyser.metadata_extraction import extract_metadata
from flowcytometer_tool.tabs.continuous_sample_analyser.json_safe import json_safe
import os
from flowcytometer_tool.tabs.blob_tools.flowcytosender import send_to_dashboard
import json
...
def _resolve_model_settings_path_for_predictions(predictions_csv=None):
    """Resolve modeltrainsettings.json from the active loaded model folder.

    If the predictions file contains calibrated feature columns, prefer the
    bead-calibrated slot. Otherwise prefer the raw/uncalibrated slot.
    """
    prefer_beadcalibrated = False
    try:
        if predictions_csv and os.path.exists(predictions_csv):
            header = pd.read_csv(predictions_csv, nrows=0).columns
            prefer_beadcalibrated = any(str(c).endswith("_calibrated") for c in header)
    except Exception:
        prefer_beadcalibrated = False
    ...
    resolvers = (
        ("bead-calibrated", resolve_active_beadcalibrated_model_path),
        ("raw/uncalibrated", resolve_active_raw_model_path),
    )
    if not prefer_beadcalibrated:
    ...
    for label, resolver in resolvers:
        try:
            ...
            if not model_path:
                continue
            settings_path = Path(model_path).with_name("modeltrainsettings.json")
            if settings_path.exists():
                return settings_path
            errors.append(f"{label}: expected {settings_path}")
        except Exception as e:
            errors.append(f"{label}: {e}")

    raise RuntimeError(
        "Could not resolve modeltrainsettings.json from the active raw/uncalibrated "
        "or bead-calibrated model folders. " + "; ".join(errors)
    )
...
def update_after_file(
    ...
    print('instrument = full_js.get("instrument", {})')
    instrument = full_js.get("instrument", {})

    measurement_settings = instrument.get("measurementSettings", {})
    cyto_settings = measurement_settings.get("CytoSettings", {})
    early_protocol_packet = {
    ...
    early_protocol = detect_sampling_protocol(early_protocol_packet)
    file_prefix = Path(cyz_json_path).stem.replace(".cyz", "")

    # Bead files define runtime calibration state. Handle them before any model
    # compatibility checks or classification-dependent QC, because bead files are
    # calibration resources rather than ordinary biological samples.
    if early_protocol == "beadsprotocol":
        bead_df = _load_csv(listmode_csv) if listmode_csv else None
        if bead_df is None and predictions_csv:
            bead_df = _load_csv(predictions_csv)
        meta = extract_metadata(cyz_json_path=cyz_json_path, predictions_df=None)
        early_protocol_packet.update(meta or {})
        updates = run_protocol_postprocessing(
            protocol="beadsprotocol",
            packet=early_protocol_packet,
            dataframe=bead_df,
            file_id=file_prefix,
        ...
        packet = {
            **(meta or {}),
            "file_id": file_prefix,
            "samplingprotocol": "beadsprotocol",
            **updates,
        }
    ...
    # ------------------------------------------------------------
    # Load the model's train settings from the active model folder actually in use.
    model_settings_path = _resolve_model_settings_path_for_predictions(predictions_csv)
    print('model_settings_path')
    print(model_settings_path)
    ...
    print('expected_serial = model_settings["instrument"]["serialNumber"]')
    ...
    packet.update(extract_prediction_calibration_packet_updates(preds_df))

    apply_sampling_protocol_mutations(packet)
    protocol = packet.get("samplingprotocol", "unknownprotocol")

    
    # ------------------------------------------------------------
    # Protocol-specific postprocessing hooks
    # ------------------------------------------------------------
    if protocol == "imageprotocol":
