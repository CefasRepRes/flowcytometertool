...
import datetime
import pickle
from flowcytometer_tool.misc.functions import active_model_dir, resolve_active_model_path 
...
def splitTrainingSet(training_set, target_name, group_name, weight_name):
    ...
    columns_to_drop = [  c for c in training_set.columns  if (c in {target_name,group_name,weight_name,"Particle_ID","TriggerLevel","sample_weight",}or "_over_" in str(c)  )]    
...
def calibrateClassifier(fitted_final_classifier, validation_set, target_name, group_name, weight_name, cores, filename_finalCalibratedModel):
  ...
  print("Tuning classifier scores (to be probabilistic) ...")
  ...
  filename_finalCalibratedModel = os.path.join(os.path.dirname(filename_finalCalibratedModel), f"final_model_{timestamp}.probabilistic.pkl")
...
def loadClassifier(model_dir):
  ...
  model_files = [
    f for f in glob.glob(os.path.join(model_dir, "*.pkl"))
    if not f.endswith("probabilistic.pkl") # For some reason this is causing an error: 'CalibratedClassifierCV' object is not subscriptable
  ]
...
def load_classifier_from_exact_path(model_path):
...
def predictPhyto(model_path, predict_name, data):
  ...
  fitted_final_classifier, classes, features = loadClassifier(model_dir)
...
def getPermutationImportance(
    ...
    def _extract_feature_names(estimator):
        ...
        try:
            ...
            if calibrated_classifiers:
                for calibrated in calibrated_classifiers:
                    found = _extract_feature_names(calibrated)
                    if found:
                        return found

                    for attr in ("estimator", "classifier", "base_estimator"):
                        wrapped = getattr(calibrated, attr, None)
                        found = _extract_feature_names(wrapped)
                        if found:
                            return found
        except Exception:
    ...
    if not model_path or not os.path.isfile(model_path):
        raise FileNotFoundError(
            f"Permutation importance model_path does not exist: {model_path}"
        )

    print("[permutation importance] loading model used for this training run:")
    print(model_path)

    fitted_final_classifier = joblib.load(model_path)
    features = _extract_feature_names(fitted_final_classifier)

    if not features:
        print(
            "[permutation importance] warning: could not read feature_names_in_ "
            "from the fitted model. Falling back to dataframe feature columns."
        )
        ...
        features = [c for c in data.columns if c not in metadata_cols]

    print("[permutation importance] model/data feature columns:")
    print(features)

    missing_features = [c for c in features if c not in data.columns]
    if missing_features:
        print("[permutation importance] incoming data columns:")
        print(list(data.columns))
        raise KeyError(
            "Permutation importance cannot run because the model expects "
            "columns missing from the current training dataframe. "
    ...
    permutation = permutation_importance(
        estimator=fitted_final_classifier,
        X=X,
        y=y,
        scoring=scorer,
    ...
    importances = pd.DataFrame(
        permutation.importances[sorted_importances_idx].T,
    ...
    plot_filename = os.path.join(plots_path, "top10.png")
    plt.savefig(plot_filename)
    plt.close()

    print(f"[permutation importance] plot written: {plot_filename}")
